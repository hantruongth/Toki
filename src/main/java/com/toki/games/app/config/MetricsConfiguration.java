...
public class MetricsConfiguration extends MetricsConfigurerAdapter implements ServletContextInitializer {
    ...
    public MetricsConfiguration(DefinedProperties definedProperties, CacheManager cacheManager) {
    ...
    public void init() {
        ...
        if (definedProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }
        if (definedProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                ...
                .build();
            reporter.start(definedProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }
    ...
    public void onStartup(ServletContext servletContext) {
        ...
        if (definedProperties.getMetrics().getPrometheus().isEnabled()) {
            String endpoint = definedProperties.getMetrics().getPrometheus().getEndpoint();
